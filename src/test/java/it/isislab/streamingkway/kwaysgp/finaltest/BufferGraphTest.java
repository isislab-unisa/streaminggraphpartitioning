...
import it.isislab.streamingkway.graphloaders.BufferedGraphLoader;
...
extends TestCase implements HeuristicsTest
{
	...
	private static final int BUFFER_SIZE_STEP = 100;
	 ...
	 * @throws IOException 
	 */
	public BufferGraphTest( String testName ) throws IOException
	{
		super( testName );
	}
	...
	{
		return new TestSuite( BufferGraphTest.class );
	}
	...
	private void allHeuristicsTestCompareSimple(File fpIn, File fpOut, Integer k, Integer C, 
		...
		ArrayList<Integer> allHeuristics = new ArrayList<Integer>(1);
		...
		for (Integer i : allHeuristics) {
			...
			File fpInTest = null;
			for (int i1 = 1; i1 < BUFFER_MAX_SIZE; i1+=BUFFER_SIZE_STEP) {
				for (int j = 0; j < ITERATION_TIME; j++) {
					...
					gl = new BufferedGraphLoader(new FileInputStream(fpInTest), 
							new FileOutputStream(fpOut), k, heuristic, C, false, i1);
				...
				String[] metrics = getMetrics(k, graphName, ord, heuristicEdgesRatio, cuttedEdges, displacement,
						normalizedMaxLoad, totalTime, heuristic, totalNodes, totalEdges, iotime, partTime, i1);
				saveCSV(metrics);
			}
	...
	private String[] getMetrics(Integer k, String graphName, String ord, ArrayList<Double> heuristicEdgesRatio,
			...
			ArrayList<Long> iotime, ArrayList<Long> parttime, Integer bufferSize) {
