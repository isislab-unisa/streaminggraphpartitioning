...
public abstract class AbstractRandomizedGreedy implements SGPHeuristic,WeightedHeuristic {
	...
	public Integer getIndex(Graph g, PartitionMap partitionMap, Node n) {
		...
		Stream<Entry<Integer,Set<Node>>> parallelStream = partitions.entrySet().parallelStream();
		parallelStream.forEach(
				new Consumer<Entry<Integer, Set<Node>>>() {
					public void accept(Entry<Integer, Set<Node>> t) {
						int index = t.getKey();
