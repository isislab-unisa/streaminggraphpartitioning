...
public abstract class AbstractCompleteNormDispersionBased  implements SGPHeuristic, WeightedHeuristic{
	...
	public Integer getIndex(PartitionMap partitionMap, Node n) {
		...
		nNeighIt.forEachRemaining(p -> {
			...
			int emb = cuv.size() == 0 ? 1 : cuv.size();
			nodeScores.put(p, 1+ Dispersion.getDispersion(p, n, dist,cuv)/emb);

		});
		nodeScores.keySet().parallelStream()
			.forEach(p -> p.getNeighborNodeIterator()
					.forEachRemaining(q -> {
						if (nodeScores.containsKey(q)) return;
						List<Node> cuv = Dispersion.cuvCalculator(q, n);
						int emb = cuv.size() == 0 ? 1 : cuv.size();
						nNeighNeigh.put(q, Dispersion.getDispersion(q, n, dist,cuv)/emb);
					}));
